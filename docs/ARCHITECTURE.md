...
- **File Storage**: Local file system only. Signatures must be stored in `server/uploads/signatures/`. 
- **Prohibited Tech**: Supabase, Docker, external Cloud databases, third-party authentication services, external storage (S3).
...
2. **DIRECTOR**: Can view PENDING_APPROVAL vouchers, review details, and either approve or reject them.
...
- `server/`: Express API, Prisma schemas and migrations, file upload endpoints.
